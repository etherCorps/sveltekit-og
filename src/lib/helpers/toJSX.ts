...
export function createVNode(element: string | Component, componentProps?: ComponentOptions): VNode {
	return handleSync(
		() => {
			if (typeof element === "string") return html(element.replaceAll("\n", "").trim());
			const { body, head } = renderComponentToHtml(element, componentProps?.props);
